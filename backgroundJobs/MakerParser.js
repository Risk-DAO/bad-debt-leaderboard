...
class MakerParser {
  constructor(web3, heavyUpdateInterval = 5) {
    ...
    this.spotter = new web3.eth.Contract(
      Addresses.spotterAbi,
      Addresses.vatEthAddress['ETH'].spotterAddress
    );
    ...
    this.multicall = new web3.eth.Contract(
      ...
      Addresses.multicallAddress['ETH']
    );
  ...
  async collectAllUsers() {
    ...
    const sig0 = this.web3.utils
      .keccak256('fork(bytes32,address,address,int256,int256)')
      .slice(0, '0x00112233'.length);
  ...
  async collectAllUsersFromEvents(chainId, eventTopic, ilkIndex, urnIndices) {
    ...
    for (
      let startBlock = this.lastUpdateBlock;
      ...
      const allEvents = await this.vat.getPastEvents('allEvents', {
        topics: [eventTopic],
