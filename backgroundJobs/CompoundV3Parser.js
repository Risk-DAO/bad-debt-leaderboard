...
class CompoundV3 {
  ...
  constructor(compoundInfo, network, web3, heavyUpdateInterval = 24, fetchDelayInHours = 1) {
    ...
    if (compoundInfo[network].nonBorrowableMarkets)
      this.nonBorrowableMarkets = compoundInfo[network].nonBorrowableMarkets;

    this.rektMarkets = [];
    if (compoundInfo[network].rektMarkets) this.rektMarkets = compoundInfo[network].rektMarkets;

    this.priceOracle = new web3.eth.Contract(
      Addresses.oneInchOracleAbi,
      Addresses.oneInchOracleAddress[network]
    );
  ...
  async initPrices() {
    ...
    for (const market of this.markets) {
      ...
      if (false && this.cETHAddresses.includes(market)) {
        // V3 works with weth, no need for special eth handling
  ...
  async updateUsers(userAddresses) {
    ...
    for (const user of userAddresses) {
      for (const market of this.markets) {
        ...
        if (market == this.baseTokenAddress || this.rektMarkets.includes(market)) {
          ...
          collatCall['callData'] = this.comet.methods
            .userCollateral(ZERO_ADDRESS, market)
            .encodeABI();
        } else {
    ...
    for (const user of userAddresses) {
      ...
      for (const market of this.markets) {
        ...
        const colatBalRet = this.web3.eth.abi.decodeParameters(
          ['uint128', 'uint128'],
          collateralBalaceResults[globalIndex].returnData
        );
        const borrowBal = this.web3.eth.abi.decodeParameter(
          'uint256',
          borrowBalanceResults[globalIndex].returnData
        );
...
async function test() {
  ...
  const comp = new CompoundV3(compoundInfo, 'ETH', web3);
