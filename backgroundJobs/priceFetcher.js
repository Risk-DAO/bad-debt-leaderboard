...
const specialAssetPriceFetchers = {
  AVAX_0x57319d41F71E81F3c65F2a47CA4e001EbAFd4F33: async (web3, network, stakedTokenAddress) => {
    ...
    const stakedTokenUnderlyingBalance = await tokenContract.methods
      .balanceOf(stakedTokenAddress)
      .call();
    ...
    return fromWei(
      toBN(stakedTokenUnderlyingBalance).mul(underlyingPrice).div(toBN(stakedTokenTotalSupply))
    );
  },
  ...
  BSC_0x695FD30aF473F2960e81Dc9bA7cB67679d35EDb7: async (web3, network, stakedTokenAddress) => {
    // renzec
    const { data } = await axios.get(
      'https://api.coingecko.com/api/v3/simple/price?ids=renzec&vs_currencies=USD'
    );
    return Number(Object.values(data)[0].usd);
  },
  ...
  ETH_0xF3A43307DcAFa93275993862Aae628fCB50dC768: async (web3, network, address) => {
    ...
    const curveContract = new web3.eth.Contract(
      PriceAddresses.curveAbi,
      PriceAddresses.cvxFXSFXSCurveAddress
    );
  ...
  ETH_0x1985365e9f78359a9B6AD760e32412f4a445E862: async (web3, network, address) => {
    ...
    return Number(
      fromWei(await getPrice(network, '0x221657776846890989a759BA2973e427DfF5C9bB', web3))
    );
  },
...
const chainTokenFetchers = {
  OPTIMISM: async () => {
    const { data } = await retry(axios.get, [
...
const get1InchPrice = async (network, address, web3) => {
  const oneInch = new web3.eth.Contract(
    Addresses.oneInchOracleAbi,
    Addresses.oneInchOracleAddress[network]
  );
  const price = await oneInch.methods
    .getRate(address, Addresses.usdcAddress[network], false)
    .call();
  return price;
};

async function testPrices() {
  const wiredTokensAvax = [
    '0x57319d41F71E81F3c65F2a47CA4e001EbAFd4F33',
    '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7',
  ];
  const wiredTokensEth = [
  ...
  for (const token of wiredTokensAvax) {
    ...
    console.log(
      ...
      { adjustedApiPrice },
      { adjustedOneInch },
      Number(adjustedApiPrice) / Number(adjustedOneInch)
    );
  }
  ...
  for (const token of wiredTokensEth) {
    ...
    console.log(
      ...
      { adjustedOneInch },
      'ratio',
      Number(adjustedOneInch) / Number(adjustedApiPrice)
    );
  }
}
...
async function getCTokenPriceFromZapper(ctoken, underlying, web3, network) {
  ...
  const normalizedUSDValue = toBN(toWei(totalBalanceInUSD))
