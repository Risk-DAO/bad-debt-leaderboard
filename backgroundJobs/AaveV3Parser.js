...
const LOAD_USERS_FROM_DISK =
  process.env.AAVEV3_LOAD_USER_FROM_DISK &&
  process.env.AAVEV3_LOAD_USER_FROM_DISK.toLowerCase() == 'true';
/**
...
class AaveV3 {
  constructor(AaveV3Info, network, web3, heavyUpdateInterval = 24) {
    ...
    this.poolAddressesProviderRegistryContract = new web3.eth.Contract(
      Addresses.aaveV3poolAddressesProviderRegistryAbi,
      AaveV3Info[network].poolAddressesProviderRegistry
    );

    console.log(
      `Created aave v3 parser for network ${this.network} and poolAddressesProviderRegistry: ${AaveV3Info[network].poolAddressesProviderRegistry}`
    );
    this.multicall = new web3.eth.Contract(
      Addresses.multicallAbi,
      Addresses.multicallAddress[network]
    );
  ...
  async init() {
    ...
    const poolProviderAddresses = await this.poolAddressesProviderRegistryContract.methods
    ...
    const poolAddressProviderV3Contract = new this.web3.eth.Contract(
      Addresses.aaveV3poolAddressProviderAbi,
      poolProviderAddresses[0]
    );
    ...
    const priceOracleContract = new this.web3.eth.Contract(
      Addresses.aaveV3PriceOracleABI,
      priceOracleAddress
    );
  ...
  async getPastEventsInSteps(contract, key, from, to) {
    ...
    console.log(
      `getPastEventsInSteps[${key}]: found ${totalEvents.length} events from ${from} to ${to}`
    );
    return totalEvents;
  }
  ...
  async periodicUpdateUsers(lastUpdatedBlock) {
    ...
    for (const key of keys) {
      ...
      const newEvents = await this.getPastEventsInSteps(
        this.lendingPool,
        key,
        lastUpdatedBlock,
        currBlock
    ...
    console.log(
      `periodicUpdateUsers: Update userList from ${userListPreviousLength} to ${this.userList.length} users`
    );
  ...
  async collectAllUsers() {
    ...
    if (LOAD_USERS_FROM_DISK) {
      ...
      if (fs.existsSync(`saved_data/${dataFileName}`)) {
        ...
        if (savedData.lastFetchedBlock && savedData.users) {
          ...
          console.log(
            `collectAllUsers: Loaded user list from disk, next block to fetch: ${firstBlockToFetch}. Current userList.length: ${this.userList.length}.`
          );
        }
      } else {
        console.log(
          `Could not find saved data file saved_data/${dataFileName}, will fetch data from the begining`
        );
      }
    }
    ...
    while (lastBlockFetched < currBlock) {
      ...
      const endBlock =
        startBlock + currentStep - 1 > currBlock ? currBlock : startBlock + currentStep - 1;

      let events;
      try {
        events = await this.lendingPool.getPastEvents('Supply', {
          fromBlock: startBlock,
          toBlock: endBlock,
        });
        console.log(
          `collectAllUsers: block ${startBlock} -> ${endBlock}. Found ${events.length} events in ${currentStep} blocks. Current userList.length: ${this.userList.length}.`
        );
      } catch (err) {
        console.log(
          `call failed when fetching events from block ${startBlock} -> ${endBlock}, trying again`,
          err.toString()
        );
    ...
    console.log(
      `collectAllUsers: collecting ${this.userList.length} users took ${Math.round(
        (Date.now() - dtCollectStart) / 1000
      )} s`
    );
  }
  ...
  async updateAllUsers() {
    ...
    console.log(
      `updateAllUsers: updated ${this.userList.length} users took ${Math.round(
        (Date.now() - dtUpdateStart) / 1000
      )} s`
    );
  ...
  async calcBadDebt(currTime) {
    ...
    this.output = {
      total: this.sumOfBadDebt.toString(),
      updated: currTime.toString(),
      decimals: '8',
      users: userWithBadDebt,
  ...
  async updateUsers(userAddresses) {
    ...
    const getUserAccountResults = await this.multicall.methods
      .tryAggregate(false, getUserAccountCalls)
      .call();
