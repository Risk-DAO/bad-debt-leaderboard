...
class MimParser {
  constructor(mimInfo, network, web3, heavyUpdateInterval = 24) {
    ...
    if (!this.covalentApiKey) {
      ...
      console.log(
        `Covalent api key is set with value: ${this.covalentApiKey.slice(
          0,
          6
        )}[...]${this.covalentApiKey.slice(-4)}`
      );
    }
  }
  ...
  async initPrices() {
    for (const calderon of this.calderons) {
      ...
      this.bentobox[calderon.options.address] = new this.web3.eth.Contract(
        Addresses.bentoboxAbi,
        bentoBoxAddress
      );
  ...
  async collectAllUsers() {
    ...
    const addCollateralTopic = this.web3.utils.keccak256(
  ...
  async collectAllUsersFromEvents(chainId, eventTopic, userTopicIndex) {
    ...
    for (
      let startBlock = this.lastUpdateBlock[calderonAddress];
      startBlock < currBlock;
      startBlock += this.blockStepInInit
    ) {
      console.log({ startBlock }, this.userList.length, this.blockStepInInit);
      const endBlock =
        startBlock + this.blockStepInInit > currBlock
          ? currBlock
          : startBlock + this.blockStepInInit;
    ...
    for (const calderon of this.calderons) {
      ...
      for (const key of keys) {
        ...
        const newEvents = await this.getPastEventsInSteps(
          calderon,
          key,
          this.lastUpdateBlock[calderon.options.address],
          currBlock
        );
  ...
  async calcBadDebt(currTime) {
    ...
    for (const user of Object.keys(this.users)) {
      ...
      for (const calderon of Object.keys(this.users[user])) {
        ...
        const collateraValue = toBN(data.collateral)
          .mul(toBN(toWei('1')))
          .div(toBN(price));
        const debt = toBN(data.debt)
          .mul(toBN(this.elastic[calderon]))
          .div(toBN(this.base[calderon]));
  ...
  async updateUsers(userAddresses) {
    ...
    const collateralBalaceResults = await this.multicall.methods
      .tryAggregate(false, collateralBalanceCalls)
      .call();
    console.log('getting borrow balances');
    const borrowBalanceResults = await this.multicall.methods
      .tryAggregate(false, borrowBalanceCalls)
      .call();
    ...
    for (const collatResult of collateralBalaceResults) {
      for (const calderon of this.calderons) {
        ...
        call['callData'] = this.bentobox[calderon.options.address].methods
          .toAmount(this.collateral[calderon.options.address], colatBal.toString(), false)
          .encodeABI();
    ...
    const collateralAmountResults = await this.multicall.methods
      .tryAggregate(false, toAmountCalls)
      .call();
    ...
    for (const user of userAddresses) {
      ...
      const colatBal = this.web3.eth.abi.decodeParameter(
        'uint256',
        collateralAmountResults[userIndex].returnData
      );
      const borrowBal = this.web3.eth.abi.decodeParameter(
        'uint256',
        borrowBalanceResults[userIndex].returnData
      );
      ...
      for (const calderon of this.calderons) {
        ...
        this.users[user][calderon.options.address] = {
          debt: this.web3.utils.toBN(borrowBal),
          collateral: this.web3.utils.toBN(colatBal),
        };
      }
    }
  }
