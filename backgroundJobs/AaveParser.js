...
class Aave {
  constructor(AaveInfo, network, web3, heavyUpdateInterval = 24) {
    ...
    this.lendingPoolAddressesProvider = new web3.eth.Contract(
      Addresses.lendingPoolAddressesProviderAbi,
      AaveInfo[network].lendingPoolAddressesProviderAddress
    );
  ...
  async initPrices() {
    ...
    const lendingPoolAddress = await this.lendingPoolAddressesProvider.methods
      .getLendingPool()
      .call();
  ...
  async collectAllUsers() {
    const currBlock =
      /*this.deployBlock + 5000 * 5 //*/ (await this.web3.eth.getBlockNumber()) - 10;
    console.log({ currBlock });
    for (
      ...
      try {
        ...
        events = await this.lendingPool.getPastEvents('Deposit', {
  ...
  async updateUsers(userAddresses) {
    ...
    for (let i = 0; i < userAddresses.length; i++) {
      ...
      if (user === '0x4846AEe6d7C9f176F3F329E01A014c2794E21B92')
