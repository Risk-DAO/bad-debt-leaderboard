...
class Compound {
  ...
  constructor(compoundInfo, network, web3, heavyUpdateInterval = 24, fetchDelayInHours = 1) {
    ...
    this.comptroller = new web3.eth.Contract(
      Addresses.comptrollerAbi,
      compoundInfo[network].comptroller
    );
  ...
  async collectAllUsers() {
    ...
    for (
      ...
      try {
        ...
        events = await this.comptroller.getPastEvents('MarketEntered', {
  ...
  async updateUsers(userAddresses) {
    ...
    for (const user of userAddresses) {
      ...
      const assetsIn = this.web3.eth.abi.decodeParameter(
        'address[]',
        assetInResult[userIndex].returnData
      );
      userIndex++;
      ...
      const userData = new User(
        user,
        this.intersect(assetsIn, this.markets),
        borrowBalances,
        collateralBalances,
        !success
      );
