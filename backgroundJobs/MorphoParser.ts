...
export abstract class MorphoParser<T> implements IParser {
  protected provider = MulticallWrapper.wrap(
  ...
  async main(): Promise<any> {
    try {
      ...
      const [currentBlock] = await Promise.all([this.provider.getBlock('latest'), this.init()]);
    ...
    console.log(
      `${this.filename.replace('.json', '')}: waiting ${waitTime} seconds before next run`
    );
  ...
  #computeBadDebt(timestamp: number) {
    ...
    const { tvl, borrows, deposits, total } = Object.entries(this.#users).reduce(
      (acc, [address, user]) => {
  ...
  async #heavyUpdate(block: number) {
    ...
    for (
      let i = 0;
      i < this.#usersList.length;
      i += this.#configuration.multicallBatchSize * this.#configuration.multicallRange
    ) {
      const allUsers = await Promise.all(
        Array.from({ length: this.#configuration.multicallBatchSize }).map(async (_, j) => {
          const users = this.#usersList.slice(
            i + j * this.#configuration.multicallRange,
            i + (j + 1) * this.#configuration.multicallRange
          );
  ...
  async #fetchChunkUserStatus(users: string[], block: number) {
    ...
    const userStatus = await Promise.all(
      users.map(async (user) => this.getUserBalanceStates(user, block))
    );
  ...
  abstract formatUserBalanceStates(userBalanceStates: T): User;
  ...
  async #collectAllUsers(currentBlock: number) {
    ...
    console.timeLog(
      'collectAllUsers',
      `from block ${this.lastFetchedBlock} to block ${currentBlock}`
    );
    ...
    while (block < currentBlock) {
      ...
      const users = await this.fetchUsersFromSupplyEvents(block, blockTo).catch((err) => {
        console.error(
          `collectAllUsers: error fetching users from block ${block} to block ${blockTo}`,
          err
        );
  ...
  #dumpToDisk() {
    ...
    if (!fs.existsSync(this.utils.getPath('')))
      fs.mkdirSync(this.utils.getPath(''), { recursive: true });
    fs.writeFileSync(
      this.utils.getPath(this.filename),
      JSON.stringify(
        {
  ...
  #loadFromDisk() {
    ...
    const data = JSON.parse(fs.readFileSync(filename).toString()) as {
...
export abstract class MorphoParserV1<T> extends MorphoParser<T> {
  ...
  getUsersWithInterractions(block: number) {
    ...
    return Promise.all([
      this.morpho
        .queryFilter(this.morpho.filters.Supplied(), this.lastFetchedBlock + 1, block)
        .then((r) => r.map((e) => e.args._onBehalf)),
      this.morpho
        .queryFilter(this.morpho.filters.Withdrawn(), this.lastFetchedBlock + 1, block)
        .then((r) => r.map((e) => e.args._supplier)),
      this.morpho
        .queryFilter(this.morpho.filters.Borrowed(), this.lastFetchedBlock + 1, block)
        .then((r) => r.map((e) => e.args._borrower)),
      this.morpho
        .queryFilter(this.morpho.filters.Repaid(), this.lastFetchedBlock + 1, block)
        .then((r) => r.map((e) => e.args._onBehalf)),
  ...
  async fetchUsersFromSupplyEvents(blockFrom: number, blockTo: number) {
    const supplyEvents = await this.morpho.queryFilter(
      this.morpho.filters.Supplied(),
      blockFrom,
      blockTo
    );
    const users = supplyEvents
      .map((event) => event.args?._onBehalf)
