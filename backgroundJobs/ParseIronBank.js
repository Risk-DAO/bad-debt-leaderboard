...
class IronBankParser extends Compound {
  ...
  async additionalCollateralBalance(userAddress) {
    if (userAddress === '0x560A8E3B79d23b0A525E15C6F3486c6A293DDAd2') {
      ...
      const alphaTokenCollateralValue = await this.balanceValue(
        alphaTokenAddress,
        alphaEscrowAddress
      );
      ...
      for (const [user, data] of Object.entries(this.users)) {
        if (user == exploiterContractAddress) {
          exploiterContractData = new User(
            user,
            data.marketsIn,
            data.borrowBalance,
      ...
      const exploiterContractNetValue = exploiterContractData.getUserNetValue(
        this.web3,
        this.prices
      );
