...
const runJob = (job, retry = 0) => {
  const backgroundJob = fork(
    './dist/backgroundJobs/jobRuner.js',
    ['-f', job.file, '-n', job.name, '-i', job.i],
    { silent: true }
  );
  console.log(
    `--> job start "node ./dist/backgroundJobs/jobRuner.js -f ${job.file} -n ${job.name} -i ${job.i}"`
  );
  ...
  backgroundJob.on('exit', (code) => {
    ...
    console.log(
      `--X job died "node ./dist/backgroundJobs/jobRuner.js -f ${job.file} -n ${job.name} -i ${job.i}"`
    );
    if (retry < 100) {
      // preventing an infinite loop
      retry++;
      console.log(
        `--> job restart #${retry} "node ./dist/backgroundJobs/jobRuner.js -f ${job.file} -n ${job.name} -i ${job.i} "`
      );
      // recursion
...
const init = async () => {
  ...
  for (let job of jobs) {
    ...
    if (job.multiple) {
      ...
      subJobs = subJobs.map((subJob, i) =>
        Object.assign({}, job, { name: job.name + '_' + subJob.name, i })
      );
